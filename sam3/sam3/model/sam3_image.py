...
class Sam3ImageOnVideoMultiGPU(Sam3Image):
    ...
    def _build_multigpu_buffer_next_chunk(
        ...
        if self.gather_backbone_out:
            ...
            # cast the SAM2 backbone features for all-gather (bf16 on CUDA, float32 on CPU/MPS)
            from sam3.model.device_utils import feature_storage_dtype

            backbone_fpn_bf16 = [
                x.to(feature_storage_dtype()) for x in feats["backbone_fpn"]
            ]
