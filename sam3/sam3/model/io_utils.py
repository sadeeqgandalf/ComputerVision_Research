...
from sam3.model.device_utils import to_compute_device
...
def _storage_dtype() -> torch.dtype:
    """Float16 storage is for CUDA; on CPU/MPS keep float32 to avoid bf16/float mismatches."""
...
def load_resource_as_video_frames(
    ...
    if isinstance(resource_path, list):
        ...
        img_mean = torch.tensor(img_mean, dtype=storage_dtype)[:, None, None]
        # pyrefly: ignore [bad-assignment]
        img_std = torch.tensor(img_std, dtype=storage_dtype)[:, None, None]
        ...
        for img_pil in resource_path:
            ...
            img = img.to(dtype=storage_dtype)
        ...
        if not offload_video_to_cpu:
            images = to_compute_device(images)
...
def load_image_as_single_frame_video(
    ...
    storage_dtype = _storage_dtype()
    images = images.unsqueeze(0).to(dtype=storage_dtype)
    ...
    if not offload_video_to_cpu:
        ...
        img_mean = to_compute_device(img_mean)
        # pyrefly: ignore [missing-attribute]
        img_std = to_compute_device(img_std)
    # normalize by mean and std
    # pyrefly: ignore [unsupported-operation]
    images -= img_mean
    # pyrefly: ignore [unsupported-operation]
...
def load_video_frames_from_image_folder(
    ...
    # float16 on CUDA; float32 on CPU/MPS to keep dtypes aligned with weights
    images = torch.zeros(num_frames, 3, image_size, image_size, dtype=storage_dtype)
...
def load_video_frames_from_video_file_using_cv2(
    ...
    if not offload_video_to_cpu:
        video_tensor = to_compute_device(video_tensor)
        # pyrefly: ignore [missing-attribute]
        img_mean = to_compute_device(img_mean)
        # pyrefly: ignore [missing-attribute]
        img_std = to_compute_device(img_std)
