...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        # Disable autocast in FFN on the tensor's device (cuda/mps/cpu).
        device_type = tgt.device.type if tgt.device.type in ("cuda", "mps", "cpu") else "cpu"
    ...
    def __init__(
        ...
        if boxRPB != "none":
            ...
            if resolution is not None and stride is not None:
                # Precompute on CPU; moved to reference_boxes.device in _get_rpb_matrix.
...
class SimpleRoPEAttention(nn.Module):
    ...
    def __init__(
        ...
        # Init on CPU; forward() moves freqs_cis to q.device.
