...
class Sam3VideoBase(nn.Module):
    ...
    def run_backbone_and_detection(
        ...
        tracker_backbone_fpn = [
            # Cast to weight dtype — CUDA autocast may leave bf16 feats; CPU has no autocast.
            sam_mask_decoder.conv_s0(
                sam3_image_out["tracker_backbone_fpn_0"].to(
                    dtype=sam_mask_decoder.conv_s0.weight.dtype
                )
            ),
            sam_mask_decoder.conv_s1(
                sam3_image_out["tracker_backbone_fpn_1"].to(
